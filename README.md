...
- [Bagisto](https://github.com/bagisto/bagisto) - [php-fpm](https://hub.docker.com/_/php?tab=tags&page=1&name=fpm)
...
- [Elasticsearch](https://hub.docker.com/r/elastic/elasticsearch)
- [Kibana](https://hub.docker.com/r/elastic/kibana)
...
```
git clone https://github.com/damalis/full-stack-nginx-bagisto-for-everyone-with-docker-compose.git
```

Open a terminal and `cd` to the folder in which `docker-compose.yml` is saved and run:

```
cd full-stack-nginx-bagisto-for-everyone-with-docker-compose
chmod +x install.sh
...
[Understanding and Optimizing vm.max_map_count for Elasticsearch Performance](https://opster.com/guides/elasticsearch/capacity-planning/elasticsearch-vm-max-map-count/)
...
<tbody>
  ...
  <tr>
    <td>remotehost</td>
    <td><code>certbot certonly --webroot --webroot-path /tmp/acme-challenge --rsa-key-size 4096 --non-interactive --agree-tos --no-eff-email --force-renewal --email ${LETSENCRYPT_EMAIL} -d ${DOMAIN_NAME} -d www.${DOMAIN_NAME} -d mail.${DOMAIN_NAME} -d kibana.${DOMAIN_NAME}</code></td>
  </tr>
</tbody>
</table>
...
The containers are now built and running. You should be able to access the Bagisto with the configured IP in the browser address. `https://DOMAIN_NAME`.
...
You should see the "Demo store" page in your browser. If not, please check if your PHP installation satisfies Bagisto's requirements.
...
```
docker container restart bagisto
```

add and/or remove base bagisto/php-fpm themes, plugins or custom code folders and files with any ftp client program to ./bagisto folder
<br /><br />contains your bagisto’s base configuration details, such as database connection information. You can set custom configuration for your bagisto in this file.
...
Bagisto ; installations must be configured to use Elasticsearch.

[Elasticsearch Guide](https://www.elastic.co/guide/en/elasticsearch/reference/7.17/index.html)

#### Kibana

Kibana (Elasticsearch UI): https://kibana.DOMAIN_NAME
