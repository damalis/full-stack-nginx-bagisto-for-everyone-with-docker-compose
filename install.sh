...
echo "======================================================================="
echo "|                                                                     |"
echo "|     full-stack-nginx-bagisto-for-everyone-with-docker-compose       |"
echo "|                       by Erdal ALTIN                                |"
echo "|                                                                     |"
echo "======================================================================="
sleep 2
...
then
	...
	sudo -- sh -c -e "grep -qxF '127.0.1.1  kibana.$domain_name' /etc/hosts || echo '127.0.1.1  kibana.$domain_name' >> /etc/hosts"
else
	domain_name=""
...
then
	...
	ssl_snippet="certbot certonly --webroot --webroot-path \/tmp\/acme-challenge --rsa-key-size 4096 --non-interactive --agree-tos --no-eff-email --force-renewal --email \$\{LETSENCRYPT_EMAIL\} -d \$\{DOMAIN_NAME\} -d www.\$\{DOMAIN_NAME\} -d mail.\$\{DOMAIN_NAME\} -d kibana.\$\{DOMAIN_NAME\}"
fi
...
sudo \cp env.bagisto .env.bagisto
...
sed -i 's/example.com/'$domain_name'/' .env.bagisto
sed -i 's/email@domain.com/'$email'/' .env
sed -i "s/ssl_snippet/$ssl_snippet/" .env
sed -i 's/which_db/'$which_db'/g' .env
sed -i 's/which_db/'$which_db'/g' .env.bagisto
sed -i 's/db_username/'$db_username'/g' .env
sed -i 's/db_username/'$db_username'/g' .env.bagisto
sed -i 's/db_password/'$db_password'/g' .env
sed -i 's/db_password/'$db_password'/g' .env.bagisto
sed -i 's/db_name/'$db_name'/' .env
sed -i 's/db_name/'$db_name'/' .env.bagisto
sed -i 's/db_table_prefix/'$db_table_prefix'/' .env.bagisto
...
sed -i 's/local_timezone/'$local_timezone'/' .env.bagisto
...
if [ -x "$(command -v docker)" ] && [ "$(docker compose version)" ]; then
	...
	# installing Bagisto and the other services
	docker compose up -d & export pid=$!
	echo "Bagisto and the other services installing proceeding..."
	echo
	wait $pid
	if [ $? -eq 0 ]
	then
		...
		echo
		echo "Error! could not installed Bagisto and the other services with docker compose" >&2
		echo
		exit 1
	fi
else
